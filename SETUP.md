...
**Correction.** An earlier draft of this file claimed 224 was a hard requirement
for Prithvi and TerraMind. That was wrong. Both accept 256; their `img_size`
attribute records the *pretraining* size, not a constraint, because their
positional encodings are generated per input rather than stored as a fixed
learned table. Verified by forward pass.
...
### GSD: not a problem — retracted

An earlier version of this file warned that Prithvi's 30 m HLS pretraining made
feeding it 10 m Sentinel-2 a "3x scale shift" confound. **That warning was
wrong and is withdrawn.** The official sources contradict it:
...
- **TerraMind's official multitemporal crop config
  ...
  `S2L2A` modality**, the reverse of the direction I claimed was required. Its
...
Two incidental confirmations from that TerraMind config, both favourable to the
current code: it selects encoder layer indices `[2,5,8,11]` for the base model,
...
With the GSD concern withdrawn, the case is simply that the defaults already give
...
### Do NOT re-export — retracted

An earlier version of this file said the "median 27.5% of discarded area" made a
re-export worthwhile. **That was wrong.** The discarded area is not lost coverage:
